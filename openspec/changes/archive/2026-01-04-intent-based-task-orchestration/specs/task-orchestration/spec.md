...
## Purpose
定义意图驱动的跨系统任务流编排功能，使系统能够理解用户的整体目标，自动编排并执行跨多个系统的复杂任务流程。
...
- **复用 WorkflowExecutor**：TaskOrchestrator 可以生成 WorkflowConfig，复用现有的工作流执行器
- **集成现有系统适配器**：复用 BrowserAutomation、IDEController
- **复用 LLM 客户端**：IntentRecognizer 复用现有的 ZhipuAI 客户端进行意图识别，但是还需要支持其他大模型
- **扩展命令解析**：在 CommandParser 基础上增加意图识别层

### 意图识别实现

- **LLM 驱动**：使用 大模型分析用户消息，理解语义并匹配意图
- **定义驱动**：通过 `config/intent_definitions.yaml` 定义意图类型和参数
- **提示词工程**：构建结构化提示词，包含所有可用意图类型和参数说明
- **响应解析**：解析 LLM 返回的 JSON 格式结果（意图类型、参数、置信度）
- **置信度判定**：低于阈值（0.85）时请求用户确认
